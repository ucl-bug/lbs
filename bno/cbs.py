...
def cbs_pml(
    ...
    def den(x):
        return sum(
            [((a_ref * x) ** i) / float(factorial(i)) for i in range(N)]
        ) * factorial(N)
...
def cbs_helmholtz(field: OnGrid, k_sq: OnGrid):
...
def born_solver(
    ...
    if gt is None:
        ...
        def cond_fun(carry):
            ...
            cond_2 = (
                jnp.linalg.norm(
                    field.on_grid[pml_size:-pml_size, pml_size:-pml_size, :]
                    - gt.on_grid
                )
                / gt_norm
                > tol
...
def born_iteration(field, k_sq, src, k0, epsilon):
...
def V(field: Field, k_sq: Field, k0: object = 1.0, epsilon: object = 0.1) -> Field:
...
if __name__ == "__main__":
    ...
    solution, k_sq = born_solver(
        sos, src, omega=1.0, k0=1.0, pml_size=128, max_iter=1000, tol=1e-6, alpha=1.0
    )
