...
class ComplexBornStage(nn.Module):
  ...
  def __call__(self, x, k, grid, src):
    k0 = self.param('k0',normal(1., jnp.float32),(1,),jnp.float32)[0]
    epsilon = self.param('epsilon',normal(1., jnp.float32),(1,),jnp.float32)[0]
    ...
    # Greens func
    def G(u):
      _params = jnp.zeros(list(k0.shape[1:3]) + [1,])
