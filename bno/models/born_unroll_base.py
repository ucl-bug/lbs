...
class UnrolledBornBase(nn.Module):
  ...
  def run_with_intermediate(self, k_sq, src, unrolls):
    ...
    u = u_new
    for i in range(unrolls):
      u_new, gammas = self.born_stages[i].run_with_intermediate(k_sq, u_new, src, grid)
      u = u + u_new
...
class TunableGreens(nn.Module):
  ...
  def setup(self):
      self.k0 = self.param('k0', constant(1., jnp.float32), (1,), jnp.float32)[0]
      self.epsilon = self.param('epsilon', constant(1., jnp.float32), (1,), jnp.float32)[0]
      self.amplitude = self.param('amplitude', constant(1., jnp.float32), (1,), jnp.float32)[0]
