...
class UnrolledBorn(nn.Module):
    ...
    def __call__(self, k_sq, src, unrolls):
        ...
        u_r = self.param("u0_r", constant(0.0, jnp.float32), src.shape[1:], jnp.float32)
        u_i = self.param("u0_i", constant(0.0, jnp.float32), src.shape[1:], jnp.float32)
        u0 = u_r + 1j * u_i
...
class TunableGreens(nn.Module):
    ...
    def __call__(self, u):
        # Stage params
        k0 = self.param("k0", constant(1.0, jnp.float32), (1,), jnp.float32)[0]
        epsilon = self.param("epsilon", constant(1.0, jnp.float32), (1,), jnp.float32)[
            0
        ]
        freq_scale = self.param(
            "freq_scale", constant(1.0, jnp.float32), (1,), jnp.float32
        )[0]
